...
-- -----------------------------------------------------------------------------
declare
    l_table         varchar2(128)   := 'bsi_business';          -- <-- Insert name of table to be analyized
    l_num_values    number          := 10;                      -- <-- Insert number of most stored distinct values to be shown (0 = all)
    --
...
begin
    ...
    for r in (select c.column_id, c.column_name, c.data_type, c.data_length from sys.all_tab_columns c where upper(c.table_name) = upper(l_table)) loop
        ...
        if r.data_type in ('CLOB','BLOB') then
            ...
            dbms_output.put_line('Number of rows : ' || n);
            dbms_sql.column_value( l_cursor, 1, l_total_values );
            dbms_output.put_line('Total number of distinct values : ' || l_total_values);
            ...
            while ( dbms_sql.fetch_rows(l_cursor) > 0 ) loop
                ...
                if l_row_num <= l_total_values then
            ...
            dbms_output.put_line('                 (' || l_total_values || ')');
            dbms_output.put_line(' ');
        end if;
    end loop;
end;
